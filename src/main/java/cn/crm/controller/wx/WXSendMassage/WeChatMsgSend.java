...
public class WeChatMsgSend {
     ...
     * @param
     */
    public String getAccessToken(String corpid, String corpsecret,String code) throws IOException {
        WeChatMsgSend sw = new WeChatMsgSend();
        WeChatUrlData uData = new WeChatUrlData();
        uData.setGet_Token_Url(corpid, corpsecret);
        String resp = sw.toAuth(uData.getGet_Token_Url());
    ...
    //企业微信推送
    public String sendWXMessageToUser(String userStr,Integer application_id,String sendMessage,Map<String,String> appInfo){
        WeChatMsgSend swx = new WeChatMsgSend();
        try {
            String token = swx.getToken(appInfo.get("corpid"),appInfo.get("corpsecret"));
            String postdata = swx.createpostdata(userStr, "text", application_id, "content",sendMessage);
            String resp = swx.post(
                    "utf-8",
                     WeChatMsgSend.CONTENT_TYPE,
                    (new WeChatUrlData()).getSendMessage_Url(),
                    postdata,
                    token
            );
